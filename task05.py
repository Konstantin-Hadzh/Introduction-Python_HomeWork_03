...
def all_sum ():
    ...
    while aux == False:
        ...
        count = 0
        for part in range(len(number)):
            if number[part] == '&': # специальный символ
                aux = True
                break
            else:
                count = count + int(number[part])
        count_sum += count
        print(f'Сумма всех введеных ранее чисел равна: {count_sum}')
    print(f'Финальная сумма: {count_sum}')
all_sum()